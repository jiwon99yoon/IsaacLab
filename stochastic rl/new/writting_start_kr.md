...
dg5f는 물물체를 잘 집고 특정 위치까지 lift하는데 성공공 - table의 색이 초록색이면 성공 /table의 색이
  빨간색이면 실패라는 거 먼저 말하는 게 좋겠따. inspire는는 물체를를 table의에 가만히 놔두고 contact을 하려고
  하는 모습을 보이나 결국 실패 - 하지만, performance보면 object out of bound가 작고 fingers to object는 값이
  뛰어남. 다만, ground_contact_penalty가 있있음 : position tracking과과 good finger contact을 inpsire는 잘 학습
  못한한것으으로 판단.  
  \
