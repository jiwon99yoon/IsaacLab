...
HDR_DG5F_CFG_NEW = ArticulationCfg(
    ...
    init_state=ArticulationCfg.InitialStateCfg(
        ...
        joint_pos={
            ...
            # 1. 검지, 중지, 약지, 새끼 (Fingers 2-5) - 살짝 굽혀서 물체를 감쌀 준비
            # ORIGINAL (너무 약하게 굽힘):
            # r"rj_dg_[2-5]_1": 0.0,    # 벌림 0
            # r"rj_dg_[2-5]_2": 0.3,    # 살짝 굽힘 1
            # r"rj_dg_[2-5]_3": 0.3,    # 살짝 굽힘 2
            # r"rj_dg_[2-5]_4": 0.3,    # 살짝 굽힘
            r"rj_dg_[2-5]_1": 0.0,     # 벌림 0 (중립)
            r"rj_dg_[2-5]_2": 0.5,     # 첫 관절 적당히 굽힘 (범위 [0, ~2.0])
            r"rj_dg_[2-5]_3": 0.4,     # 두번째 관절 적당히 굽힘
            r"rj_dg_[2-5]_4": 0.4,     # 세번째 관절 적당히 굽힘

            # 2. 엄지 (Finger 1) - Opposition 자세 (물체를 향해 대치)
            # ORIGINAL (너무 벌림):
            # "rj_dg_1_1": 0.85,       # 최대 벌림에 가깝게 / 범위 [-0.38, 0.89]
            # "rj_dg_1_2": -1.0,       # 회전 / 범위 [-3.14, 0.0]
            # "rj_dg_1_3": 0.3,        # 범위 [-1.57, 1.57]
            # "rj_dg_1_4": 0.6         # 범위 [-1.57, 1.57]
            "rj_dg_1_1": 0.6,        # 적당히 벌림 (범위 [-0.38, 0.89])
            "rj_dg_1_2": -1.2,       # 회전 강화 (범위 [-3.14, 0.0])
            "rj_dg_1_3": 0.3,        # 살짝 굽힘
            "rj_dg_1_4": 0.5,        # 적당히 굽힘
        },
    ),
    ...
    actuators={
        "hdr20_dg5f_actuators": ImplicitActuatorCfg(
            ...
            stiffness={
                ...
                # 3rd TRY (여전히 stiff): r"rj_dg_[1-5]_[1-4]": 5.0,
                # 참고: Kuka-Allegro는 3.0 사용
                # FINAL: Kuka와 동일하게 설정 (손가락 무게는 크게 차이 안 남, compliance 중요!)
                r"rj_dg_[1-5]_[1-4]": 3.0,  # Kuka와 동일! (자연스러운 grasping 위해)
            },
            ...
            damping={
                ...
                # 3rd TRY (여전히 느림): r"rj_dg_[1-5]_[1-4]": 0.3,
                # 참고: Kuka-Allegro는 0.1 사용
                # FINAL: Kuka와 동일하게 설정 (빠른 반응 속도 위해)
                r"rj_dg_[1-5]_[1-4]": 0.1,  # Kuka와 동일! (민첩한 grasping 위해)
            },
