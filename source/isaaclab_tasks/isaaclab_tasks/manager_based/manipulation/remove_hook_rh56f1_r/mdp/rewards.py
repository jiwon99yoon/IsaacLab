...
def contacts(env: ManagerBasedRLEnv, threshold: float) -> torch.Tensor:
    ...
    MODIFIED for Remove Hook Task with HDR35_20 + RH56F1_R (Inspire Hand):
    - Changed from DG5F (20-DOF, 5 fingers x 4 links) to Inspire Hand (6-DOF under-actuated)
    ...
    # HDR35_20_RH56F1_R: Flat structure, right hand
    thumb_contact_sensor: ContactSensor = env.scene.sensors["right_thumb_4_hook_sensor"]
    index_contact_sensor: ContactSensor = env.scene.sensors["right_index_2_hook_sensor"]
    middle_contact_sensor: ContactSensor = env.scene.sensors["right_middle_2_hook_sensor"]
    ring_contact_sensor: ContactSensor = env.scene.sensors["right_ring_2_hook_sensor"]
    last_contact_sensor: ContactSensor = env.scene.sensors["right_little_2_hook_sensor"]
...
#   1. body_names 지원 (wire의 left_ring 등 특정 body 추적 가능)
...
def position_command_error_tanh_body(
    ...
    MODIFIED for Remove Hook Task:
    - Supports body_names in align_asset_cfg (e.g., left_ring body)
    ...
    if align_asset_cfg.body_ids is not None and len(align_asset_cfg.body_ids) > 0:
        # Track specific body (e.g., left_ring for wire)
...
def hook_grasped(
    ...
    Note:
        ...
        Sensor names: right_thumb_4_hook_sensor, right_index_2_hook_sensor, etc.
    """
    # Get contact forces from all fingertip sensors
    # Right hand inspire: thumb_4, index_2, middle_2, ring_2, little_2
    finger_sensors = [
        "right_thumb_4_hook_sensor",
        "right_index_2_hook_sensor",
        "right_middle_2_hook_sensor",
        "right_ring_2_hook_sensor",
        "right_little_2_hook_sensor",
    ]
