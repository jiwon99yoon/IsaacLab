...
def contacts(env: ManagerBasedRLEnv, threshold: float) -> torch.Tensor:
    ...
    # HDR35_20_RH56F1_R: Flat structure, right hand (no RH56F1_L scope)
    thumb_contact_sensor: ContactSensor = env.scene.sensors["right_thumb_4_object_s"]
    index_contact_sensor: ContactSensor = env.scene.sensors["right_index_2_object_s"]
    middle_contact_sensor: ContactSensor = env.scene.sensors["right_middle_2_object_s"]
    ring_contact_sensor: ContactSensor = env.scene.sensors["right_ring_2_object_s"]
    last_contact_sensor: ContactSensor = env.scene.sensors["right_little_2_object_s"]
