...
def contacts(
    ...
    MODIFIED for hdr035 + multiple hand types (Inspire/DG5F):
...
def success_reward(
    ...
    MODIFIED FOR hdr035+Inspire: Added contact gating to prevent "pushing without grasping" shortcut.
    Agent was achieving success by pushing object with arm instead of grasping with fingers.
    (Originally developed for HDR-DG5F, adapted for hdr035+Inspire Hand)
    ...
    # MODIFIED (hdr035+Inspire - requires contact to prevent shortcuts):
...
# Adapted for hdr035+Inspire Hand (6-DOF under-actuated)
...
def ati_excessive_force_penalty(
    ...
    THRESHOLD RATIONALE (50N):
        - hdr035 + DG5F hand weight: ~3kg hand + ~0.5kg ATI = ~3.5kg
