...
**수정된 파일**:
- `config/hdr35_20_dg5f_l/remove_hdr35_20_dg5f_l_env_cfg.py`
  - Line 84: `std: 0.1 → 1.0` (fingers_to_hook)
  - Line 106: `std: 0.02 → 0.2` (hook_to_target)
...
**비교 분석: iiwa + Allegro vs HDR35 + DG5F**

| 파라미터 | iiwa + Allegro | HDR35 + DG5F | 비고 |
|----------|----------------|--------------|------|
| 마지막 암 조인트 Kp | j7: 25, j6: 50 | j6: 1,500 | HDR35가 30~60배 높음 |
| 핸드 Kp | 3.0 | 4.0 | 유사 |
| 암 최약 조인트/핸드 비율 | 8.3:1 ~ 16.7:1 | **375:1** | **45배 불균형** |
| Action Scale | 0.1 (전 조인트) | 0.001 (전 조인트) | - |
| 핸드 토크/액션 | 3.0×0.1 = **0.3 Nm** | 4.0×0.001 = **0.004 Nm** | HDR35 핸드가 1/75 |
...
| hand | 0.1 | 4.0×0.1 = 0.4 Nm | Allegro 수준 (0.3 Nm) |
...
- `config/hdr35_20_dg5f_l/remove_hdr35_20_dg5f_l_env_cfg.py`
  ...
  scale={
      ...
      r"lj_dg_[1-5]_[1-4]": 0.1, # DG5F 핸드
  }
  ```
...
| Action scale (hand) | 0.1 → 0.001 | 0.1 | Allegro 수준 토크 (0.4 Nm) |
...
│   └── hdr35_20_dg5f_l/
│       ├── __init__.py
│       ├── remove_hdr35_20_dg5f_l_env_cfg.py  # 로봇별 설정
│       └── agents/
