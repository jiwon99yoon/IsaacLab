...
# ============================================================================
# USD STRUCTURE (UR10e + ATI + DG5F Right Hand)
# ============================================================================
# The USD file includes tip xforms under dg5f_right_new:
#   - rl_dg_palm (palm reference point)
#   - rl_dg_1_tip, rl_dg_2_tip, rl_dg_3_tip, rl_dg_4_tip, rl_dg_5_tip (fingertips)
#
# This matches HDR20-17 DG5F structure which has:
#   - rl_dg_palm, rl_dg_1_tip, rl_dg_2_tip, etc.
#
# Configuration Structure:
# 1. CONTACT SENSORS: Placed on collision links (rl_dg_1_4, rl_dg_2_4, etc.)
#    - prim_path uses slash: "dg5f_right_new/rl_dg_1_4"
#    - sensor name uses underscore: "ur10e_dg5f_right_new_rl_dg_1_4_object_s"
#
# 2. OBSERVATIONS/REWARDS: Use tip xforms for accurate fingertip positions
#    - body_names: ["rl_dg_palm", "rl_dg_.*_tip"] (regex pattern matches all tip xforms)
#    - This provides actual fingertip positions, not link origins
# ============================================================================

from isaaclab_assets.robots import UR10E_DG5F_RIGHT_CFG
...
# ============================================================================
# DG5F HAND SCENE CONFIG (1.0x object size - same as Kuka-Allegro baseline)
# ============================================================================
# DG5F is a fully-actuated 20-DOF hand, similar in size to Allegro Hand
# Object sizes are 1.0x (same as Kuka-Allegro baseline) for controlled comparison
# This allows direct comparison with Inspire Hand (0.5x) to isolate the effect
# of hand actuation structure (underactuated vs fully-actuated)
# ============================================================================

@configclass
class Dg5fSceneCfg(dexsuite.SceneCfg):
    """Scene configuration for DG5F Hand with 1.0x objects (Kuka-Allegro baseline).

    DG5F is a fully-actuated 20-DOF hand, similar in size to Allegro Hand.
    Objects are kept at 1.0x (same as Kuka-Allegro) for direct comparison.

    Comparison:
    - Kuka-Allegro (baseline): 1.0x
    - UR10e-DG5F (this config): 1.0x (same as baseline for controlled comparison)
    - UR10e-Inspire: 0.5x (scaled for smaller underactuated hand)

    This configuration enables systematic comparison of:
    - Underactuated (6-DOF Inspire) vs Fully-actuated (20-DOF DG5F)
    - With all other factors controlled (same arm, task, rewards, observations)
    """

    # Override object configuration with 1.0x scaling (Kuka-Allegro baseline)
    object: RigidObjectCfg = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/Object",
        spawn=sim_utils.MultiAssetSpawnerCfg(
            assets_cfg=[
                # Cuboids (Kuka-Allegro × 1.0 - baseline size)
                ...
                # Spheres (Kuka-Allegro × 1.0 - baseline size)
                ...
                # Capsules (Kuka-Allegro × 1.0 - baseline size)
                ...
                # Cones (Kuka-Allegro × 1.0 - baseline size)
...
@configclass
class Ur10eDg5fRightRelJointPosActionCfg:
    action = mdp.RelativeJointPositionActionCfg(asset_name="robot", joint_names=[".*"], scale=0.1)

@configclass
class Ur10eDg5fRightReorientRewardCfg(dexsuite.RewardsCfg):
...
@configclass
class Ur10eDg5fRightMixinCfg:
    rewards: Ur10eDg5fRightReorientRewardCfg = Ur10eDg5fRightReorientRewardCfg()
    actions: Ur10eDg5fRightRelJointPosActionCfg = Ur10eDg5fRightRelJointPosActionCfg()
    ...
    def __post_init__(self: dexsuite.DexsuiteReorientEnvCfg):
        ...
        # - Palm: rl_dg_base, rl_dg_palm (palm xform)
        # - 5 Fingers (20-DOF): Finger 1-5, each with 4 joints (rj_dg_1_1 ~ rj_dg_5_4)
        # - 5 Fingertips: rl_dg_1_tip, rl_dg_2_tip, rl_dg_3_tip, rl_dg_4_tip, rl_dg_5_tip
        # - Collision links: rl_dg_1_4, rl_dg_2_4, rl_dg_3_4, rl_dg_4_4, rl_dg_5_4 (last link of each finger)

        # Command body: Use rl_dg_palm xform (matching HDR20-17 DG5F structure)
        # NOTE: Isaac Lab flattens body names, so no ur10e/ prefix needed
        self.commands.object_pose.body_name = "rl_dg_palm"

        # Robot configuration
        self.scene.robot = UR10E_DG5F_RIGHT_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        ...
        # Matching HDR20-17 pattern: contact sensors on links (rl_dg_1_4, rl_dg_2_4, etc.)
        # NOTE: UR10e has "ur10e/" prefix due to extra Xform layer in USD structure
        finger_tip_body_list = [
            "ur10e/dg5f_right_new/rl_dg_1_4",    # Finger 1 (Thumb) collision link
            "ur10e/dg5f_right_new/rl_dg_2_4",    # Finger 2 (Index) collision link
            "ur10e/dg5f_right_new/rl_dg_3_4",    # Finger 3 (Middle) collision link
            "ur10e/dg5f_right_new/rl_dg_4_4",    # Finger 4 (Ring) collision link
            "ur10e/dg5f_right_new/rl_dg_5_4"     # Finger 5 (Pinky) collision link
        ]

        # Create contact sensors for each collision link
        # Sensor names use underscore (e.g., "ur10e_dg5f_right_new_rl_dg_1_4_object_s")
        # Prim paths use slash (e.g., "{ENV_REGEX_NS}/Robot/ur10e/dg5f_right_new/rl_dg_1_4")
        ...
        # DG5F: ["rl_dg_palm", "rl_dg_.*_tip"] - Isaac Lab flattens body names
        self.observations.proprio.hand_tips_state_b.params["body_asset_cfg"].body_names = [
            "rl_dg_palm",      # Palm xform
            "rl_dg_.*_tip"     # All fingertip xforms (rl_dg_1_tip ~ rl_dg_5_tip)
        ]
        ...
        # DG5F uses rj_dg_1_1 ~ rj_dg_1_4 for thumb instead
...
@configclass
class DexsuiteUr10eDg5fRightLiftEnvCfg(Ur10eDg5fRightMixinCfg, dexsuite.DexsuiteLiftEnvCfg):
    """UR10e + DG5F Right Hand Lift task with 1.0x objects (Kuka-Allegro baseline).

    Key configuration for systematic comparison:
    - Object size: 1.0x (same as Kuka-Allegro baseline)
    - Hand: 20-DOF fully-actuated DG5F Hand
    - Arm init pose: Same as Inspire (for controlled comparison)

    This enables direct comparison with Inspire Hand to isolate the effect of:
    - Underactuated (6-DOF) vs Fully-actuated (20-DOF)
    - With all other factors controlled (same arm, task, rewards, observations)
    """
    # Override scene to use DG5F-specific object scaling (1.0x baseline)
    scene: Dg5fSceneCfg = Dg5fSceneCfg(num_envs=4096, env_spacing=3, replicate_physics=False)

@configclass
class DexsuiteUr10eDg5fRightLiftEnvCfg_PLAY(Ur10eDg5fRightMixinCfg, dexsuite.DexsuiteLiftEnvCfg_PLAY):
    """UR10e + DG5F Right Hand Lift task (Play mode) with 1.0x objects."""
    # Override scene to use DG5F-specific object scaling (1.0x baseline)
    scene: Dg5fSceneCfg = Dg5fSceneCfg(num_envs=4096, env_spacing=3, replicate_physics=False)
...
@configclass
class Ur10eDg5fRightMixinCfg_FT(Ur10eDg5fRightMixinCfg):
...
class DexsuiteUr10eDg5fRightLiftEnvCfg_FT(Ur10eDg5fRightMixinCfg_FT, dexsuite.DexsuiteLiftEnvCfg):
    """UR10e + DG5F Right Lift task with ATI F/T sensor and 1.0x objects.

    Key configuration for systematic comparison:
    - Object size: 1.0x (same as Kuka-Allegro baseline)
    - Hand: 20-DOF fully-actuated DG5F Hand
    - ATI F/T sensor: Force observations (3D) and excessive force penalty
    - Arm init pose: Same as Inspire (for controlled comparison)
    """
    # Override scene to use DG5F-specific object scaling (1.0x baseline)
    scene: Dg5fSceneCfg = Dg5fSceneCfg(num_envs=4096, env_spacing=3, replicate_physics=False)
...
class DexsuiteUr10eDg5fRightLiftEnvCfg_FT_PLAY(Ur10eDg5fRightMixinCfg_FT, dexsuite.DexsuiteLiftEnvCfg_PLAY):
    """UR10e + DG5F Right Lift task (Play mode) with ATI F/T sensor and 1.0x objects."""
    # Override scene to use DG5F-specific object scaling (1.0x baseline)
    scene: Dg5fSceneCfg = Dg5fSceneCfg(num_envs=4096, env_spacing=3, replicate_physics=False)
