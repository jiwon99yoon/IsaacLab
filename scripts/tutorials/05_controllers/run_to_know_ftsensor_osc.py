...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    while simulation_app.is_running():
        ...
        if count % 500 == 0:
            ...
            if count % 10 == 0:
                print(f"Step {count}:")
                #print(f"Available body names: {robot.body_names}")
                #Available body names: ['panda_link0', 'panda_link1', 'panda_link2', 'panda_link3', 'panda_link4', 'panda_link5', 'panda_link6', 'panda_link7', 'force_sensor', 'panda_hand', 'panda_leftfinger', 'panda_rightfinger', 'panda_fingertip_centered']

                # 1. Contact force magnitude
                contact_force_magnitude = torch.norm(contact_forces.data.net_forces_w[0])
                print(f"  1. Contact force magnitude: {contact_force_magnitude:.4f} N")
                ...
                print(f"  2. Force sensor data (6-DOF): {force_sensor_data}")

                # 3. OSC로 계산한 각 관절의 joint torque (7-DOF)
                osc_joint_torques = joint_efforts[0]
                print(f"  3. OSC joint torques (7-DOF): {osc_joint_torques}")

                # 4. Force sensor로 측정한 각 관절에서의 joint torque (7개 관절의 Z축 토크)
                # 각 관절에 해당하는 링크의 6-DOF 데이터 중 5번 인덱스 (Z축 토크) 추출\
                joint_link_indices = []
                joint_names = [f"panda_link{i}" for i in range(1, 8)]
                for joint_name in joint_names:
                    try:
                        idx = robot.body_names.index(joint_name)
                        joint_link_indices.append(idx)
                        #print(f"Found {joint_name} at index {idx}")
                    except ValueError:
                        print(f"Could not find {joint_name}")

                # 그 다음 해당 인덱스들로 토크 추출
                ft_joint_torques = []
                for idx in joint_link_indices:
                    z_torque = ft_sensor_forces[0][idx, 5]  # Z축 토크
                    ft_joint_torques.append(z_torque)

                ft_joint_torques = torch.tensor(ft_joint_torques, device=sim.device)
                print(f"  4. F/T sensor joint torques (7-DOF, Z-axis): {ft_joint_torques}")

                # 5. OSC 토크와 F/T sensor 토크의 차이
                torque_difference = osc_joint_torques - ft_joint_torques
                print(f"  5. Difference (OSC - F/T): {torque_difference}")
                print(f"#########################################")  # 빈 줄 추가
                


                ################################ 각 관절 get_link_incoming_joint_force ####################################
                # # 각 관절의 전체 6DOF force/torque 데이터 수집
                # joint_ft_data = []
                # for idx in joint_link_indices:
                #     full_ft = ft_sensor_forces[0][idx]  # [6] 벡터 (Fx,Fy,Fz,Tx,Ty,Tz)
                #     joint_ft_data.append(full_ft)
                
                # # 출력
                # for i, ft_data in enumerate(joint_ft_data):
                #     joint_name = joint_names[i] if i < len(joint_names) else f"Joint_{i+1}"
                #     print(f"  {joint_name} F/T data:")
                #     print(f"    Forces [X,Y,Z]: [{ft_data[0]:.4f}, {ft_data[1]:.4f}, {ft_data[2]:.4f}]")
                #     print(f"    Torques[X,Y,Z]: [{ft_data[3]:.4f}, {ft_data[4]:.4f}, {ft_data[5]:.4f}]")
                # print(f"#########################################") 
                ...
                # f/t sensor에서 측정되는 joint force/torque 추가
                #force_sensor_body_idx = robot.body_names.index("force_sensor")
                # ft_sensor_forces = robot.root_physx_view.get_link_incoming_joint_force()
                # print(f"  F/T Sensor joint forces (env 0, all links): {ft_sensor_forces}")
                # 특정 force sensor body의 데이터만 추출
                #force_sensor_data = ft_sensor_forces[0][force_sensor_body_idx]  # [6] 벡터 (force + torque)
                
                #print(f"  OSC Joint torques (7-DOF): {joint_efforts[0]}")
                #print(f"  Force sensor data (6-DOF): {force_sensor_data}")
                #print(f"  Force sensor - Force: {force_sensor_data[:3]}")
                #print(f"  Force sensor - Torque: {force_sensor_data[3:]}")
                #print(f"  Contact force magnitude: {torch.norm(contact_forces.data.net_forces_w[0]):.4f} N")
                
                # print(f"  ENV0 F/T sensor data (6-DOF): {force_sensor_data}")
                # print(f"  F/T Sensor joint forces (env 0, all links): {force_sensor_data}")
