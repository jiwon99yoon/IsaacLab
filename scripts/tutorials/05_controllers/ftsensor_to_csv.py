...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    # 1. 로그 데이터를 저장할 리스트를 루프 시작 전에 초기화합니다.
    log_data = []
    ...
    try:
        ...
        while simulation_app.is_running():
            ...
            if count % 250 == 0:
                ...
                if count % 5 == 0:
                    # 2. print 대신 딕셔너리에 데이터를 저장합니다.
                    # 첫 번째 환경(env 0)의 데이터만 저장하는 예시입니다.
                    # .item()을 사용해 tensor 값을 일반 숫자로 변환합니다.
                    # .cpu().numpy()를 사용해 tensor를 numpy 배열로 변환할 수도 있습니다.

                    # 1. Contact force magnitude
                    contact_force_magnitude = torch.norm(contact_forces.data.net_forces_w[0])
                    # 2. Force sensor의 data (6-DOF)
                    ...
                    force_sensor_data = ft_sensor_forces[0][force_sensor_body_idx]
                    # 3. OSC로 계산한 각 관절의 joint torque (7-DOF)
                    osc_joint_torques = joint_efforts[0]

                    # 4. Force sensor로 측정한 각 관절에서의 joint torque
                    joint_link_indices = [robot.body_names.index(f"panda_link{i}") for i in range(1, 8)]
                    ft_joint_torques_list = [ft_sensor_forces[0][idx, 5] for idx in joint_link_indices]
                    ft_joint_torques = torch.tensor(ft_joint_torques_list, device=sim.device)

                    # 4. Joint 2, 4의 6-DOF 힘/토크 데이터 추출
                    joint_2_idx = robot.body_names.index("panda_link2")
                    joint_4_idx = robot.body_names.index("panda_link4")
                    joint_2_data = ft_sensor_forces[0][joint_2_idx]  # [fx, fy, fz, tx, ty, tz]
                    joint_4_data = ft_sensor_forces[0][joint_4_idx]  # [fx, fy, fz, tx, ty, tz]
                    
                    # 5. OSC 토크와 F/T sensor 토크의 차이
                    torque_difference = osc_joint_torques - ft_joint_torques
                    # 3. 데이터를 저장할 딕셔너리를 생성합니다.
                    current_log = {
                        "step": count,
                        "contact_force_magnitude": contact_force_magnitude.item(),
                        # 6-DOF, 7-DOF 데이터는 각 축별로 저장하는 것이 분석에 용이합니다.
                        "fs_force_x": force_sensor_data[0].item(),
                        "fs_force_y": force_sensor_data[1].item(),
                        "fs_force_z": force_sensor_data[2].item(),
                        "fs_torque_x": force_sensor_data[3].item(),
                        "fs_torque_y": force_sensor_data[4].item(),
                        "fs_torque_z": force_sensor_data[5].item(),

                        # Joint 2의 6-DOF 힘/토크 데이터
                        "joint2_force_x": joint_2_data[0].item(),
                        "joint2_force_y": joint_2_data[1].item(),
                        "joint2_force_z": joint_2_data[2].item(),
                        "joint2_torque_x": joint_2_data[3].item(),
                        "joint2_torque_y": joint_2_data[4].item(),
                        "joint2_torque_z": joint_2_data[5].item(),

                        # Joint 4의 6-DOF 힘/토크 데이터
                        "joint4_force_x": joint_4_data[0].item(),
                        "joint4_force_y": joint_4_data[1].item(),
                        "joint4_force_z": joint_4_data[2].item(),
                        "joint4_torque_x": joint_4_data[3].item(),
                        "joint4_torque_y": joint_4_data[4].item(),
                        "joint4_torque_z": joint_4_data[5].item(),
                    }

                    # 7개 관절 토크 데이터 추가
                    for i in range(7):
                        current_log[f'osc_torque_j{i+1}'] = osc_joint_torques[i].item()
                        current_log[f'ft_torque_j{i+1}'] = ft_joint_torques[i].item()
                        current_log[f'diff_torque_j{i+1}'] = torque_difference[i].item()
                    # 4. 생성된 딕셔너리를 log_data 리스트에 추가합니다.
        ...
        # 5. 시뮬레이션이 끝나면 (또는 중단되면) 리스트에 저장된 데이터를 CSV 파일로 저장합니다.
        print("Simulation finished. Saving log data...")
        if log_data: # 로그 데이터가 있을 경우에만 저장
            df = pd.DataFrame(log_data)
            df.to_csv("simulation_log.csv", index=False)
            print("Log data saved to simulation_log.csv")
        else:
